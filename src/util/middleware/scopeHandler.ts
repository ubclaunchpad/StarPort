...
import {Database, getDatabaseParser, queryDatabaseAPI} from '../db';
...
export class ScopeController implements IMiddleware<IHandlerEvent, object> {
    ...
    public handler = async (event: APIGatewayProxyEvent) => {
        ...
        const userQuery = await this.connection
            .selectFrom('person')
            .select(['email'])
            .where('email', '=', userEmail)
            .compile();

        const user =  (await queryDatabaseAPI(userQuery)).rows[0];
        if (!user) {
            throw new NotFoundError('User not found');
        }

        const scopesQuery = await this.connection
            ...
            .compile();
        const scopes =  (await queryDatabaseAPI(scopesQuery)).rows;
    ...
    constructor(connection?: Kysely<Database>) {
        ...
        this.connection = connection;}
            ...
            this.connection = getDatabaseParser();
        }
