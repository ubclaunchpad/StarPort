...
export const handler = async function(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
  try {
   if (event === null) {
    ...
    if (event.pathParameters === null || event.pathParameters.userId === null) {
        throw new Error('User Id is missing');
    }
  
   const resp = await updateUser(event.pathParameters.userId as string, event.body as UserUpdateI);
   mysql.end();
...
export const updateUser = async (userId: string, userInfo: UserUpdateI): Promise<void> => {
    const { email, firstName, prefName, lastName, resumeLink, facultyId, standingId, programId } = userInfo;
