...
export class PostingsStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const api: apigateway.RestApi = new apigateway.RestApi(
      ...
      components.addMethod("GET", new apigateway.LambdaIntegration(getPostings));
      ...
      const getAppStatusTypes = new lambda.Function(this, 'getAppStatusTypes', {
        runtime: lambda.Runtime.NODEJS_16_X,    // execution environment
      ...
      const getUserApplications= new lambda.Function(this, 'getUserApplications', {
        runtime: lambda.Runtime.NODEJS_16_X,    // execution environment
        code: lambda.Code.fromAsset('dist/postings/getUserApplications'),  // code loaded from "lambda" directory
        handler: 'index.handler',
        environment: {
            DB_HOST: process.env.DB_HOST!,
            DB_USERNAME: process.env.DB_USERNAME!,
      ...
      const components4 = components3.addMethod("GET", new apigateway.LambdaIntegration(getApplications));


      const components9 = components2.addMethod("GET", new apigateway.LambdaIntegration(getPosting));

      const components7 = components.addResource("applications");
      const components8 = components7.addMethod("GET", new apigateway.LambdaIntegration(getUserApplications));
      const components5 = components7.addResource("status");
      const components6 = components5.addMethod("GET", new apigateway.LambdaIntegration(getAppStatusTypes));
